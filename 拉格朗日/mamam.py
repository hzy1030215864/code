...
for t in range(T):
    ...
    # model.addConstr(e[3, 0, t] + e[3, 2, t] == 1)
...
for t in range(T):
    ...
    # model.addConstr(e[3, 0, t]*R_j[0]*l_i2jt[3,0,t]+e[3, 2, t]*R_j[2]*l_i2jt[3,2,t]>=hv_i2_min[3])
    # model.addConstr(e[3, 0, t]*R_j[0]*l_i2jt[3,0,t]+e[3, 2, t]*R_j[2]*l_i2jt[3,2,t]<=hv_i2_max[3])
...
lambda_i1jt1 = np.eye(J, T, dtype=np.float64)
x_vale =[(0,0,0,0)]
l_i1jt_values = []
epsilon = 1e-6
for iteration in range(max_iterations):
    ...
    for t in range(1,T):
        for j in range(J):
            ...
            objective_expr += lambda_i1jt1[j, t]*(h_jt[j, t]- H_j1[j] * (1 - condition[j, t])-(H_j0[j] + seita[j, t] / V_j[j]) * condition[j, t])
            objective_expr +=lambda_i1jt1[j, t]*(F_jt[j, t]-(seita[j, t] - (H_j1[j] - H_j0[j]) * V_j[j]) * (1 - condition[j, t]))
            for i1 in range(I1):
    ...
    for j in range(J):
        for t in range(1,T):
            ...
            for i2 in range(I2):
                ...
                l2_total += l2_value
            h_jt_var = h_jt[j, t]
            h_jt_value = h_jt_var.x
            h_jt1_var = h_jt[j, t - 1]
            h_jt1_value = h_jt1_var.x
            F_jt_var = F_jt[j, t]
            F_jt_value = F_jt_var.x
            O_var = O[j, t]
            O_value = O_var.x
            V_jt_var = V_jt[j, t]
            V_jt_value = V_jt_var.x
            seita_var = seita[j, t]
            seita_value = seita_var.x
            condition_var = condition[j,t]
            condition_value = condition_var.x
            erlfa1 = (h_jt_value - H_j1[j]*(1-condition_value)- (H_j0[j]+seita_value)*condition_value+F_jt_value-(seita_value-(H_j1[j]-H_j0[j]))*(1-condition_value) ) ** 2
            # erlfa2 = (R_j[j] * l_value - hv_i1_max[i])**2
            objective_value = objective_expr.getValue()
            sete[j,t] = (-2424356 - objective_value) / erlfa1
            fm = h_jt_value - H_j1[j]*(1-condition_value)- (H_j0[j]+seita_value)*condition_value+F_jt_value-(seita_value-(H_j1[j]-H_j0[j]))*(1-condition_value)
