...
for iteration in range(max_iterations):
    ...
    for t in range(1,T):
        for j in range(J):
            ...
            objective_expr += lambda_i1jt1[j, t]*(h_jt[j, t-1]+O[j,t]-H_j0[j]-V_jt[j, t]-seita[j,t] )
            for i1 in range(I1):
    ...
    for j in range(J):
        for t in range(1,T):
            ...
            erlfa1 = (h_jt1_value + O_value - H_j0[j]- l1_total - l2_total - V_jt_value-seita_value ) ** 2
            # erlfa2 = (R_j[j] * l_value - hv_i1_max[i])**2
            objective_value = objective_expr.getValue()
            sete[j,t] = (-2424356 - objective_value) / erlfa1
            fm = h_jt1_value + O_value - H_j0[j]- l1_total - l2_total - V_jt_value-seita_value
