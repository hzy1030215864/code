...
from keras.layers import LSTM, Dense, Input, Concatenate, LayerNormalization, Reshape
...
# 定义模型
model = Sequential()
model.add(Conv1D(filters=64, kernel_size=4, activation='tanh', padding='same', input_shape=(X_train.shape[1], X_train.shape[2])))
model.add(LSTM(units=128, activation='tanh', return_sequences=True))

model.add(LSTM(units=128, activation='tanh'))
model.add(Dense(units=n_output))  # 输出层神经元数量为30，用于预测30个点
