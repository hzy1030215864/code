...
model_gru.add(TCN(nb_filters=128, kernel_size=3, nb_stacks=1, dilations=[1, 2, 4, 8, 16, 32], padding='causal', use_skip_connections=True, dropout_rate=0.1, return_sequences=True, activation='tanh'))
...
# 训练模型
history = model_gru.fit(X, y, epochs=50, batch_size=32, validation_split=0.15)
...
df_result.to_excel('TCN+GRUBFG消耗.xlsx', index=False)
...
df_metrics.to_excel('TCN+GRUBFG消耗数据.xlsx', index=False)