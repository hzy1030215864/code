...
n_features = 5        # 你输入数据中的特征数量，这里以 4 个为例
n_output = 30
X, y = [], []
for i in range(len(xiaohao) - sequence_length - n_output):
    X.append(np.c_[xiaohao[i:i+sequence_length], fengya[i:i+sequence_length], fl[i:i+sequence_length], fy[i:i+sequence_length],bfgfasheng[i:i+sequence_length]])
...
# 定义模型
model = Sequential()
model.add(GRU(128, activation='tanh', return_sequences=False, input_shape=(sequence_length, n_features)))
