...
filename = r'E:\新建文件夹\代码\lstm\data\煤气消耗数据\COG消耗量-烧结球团\A1A00000401\20171019.txt'
...
# 定义模型
model = Sequential()
model.add(TCN(nb_filters=128, kernel_size=3, nb_stacks=1, dilations=[1, 2, 4, 8, 16, 32], padding='causal', use_skip_connections=True, dropout_rate=0.01, return_sequences=True, activation='tanh'))
...
df_result.to_excel('TCN+GRUCOG消耗.xlsx', index=False)
plt.legend()
...
df_metrics.to_excel('TCN+GRUCOG消耗数据.xlsx', index=False)
plt.show()