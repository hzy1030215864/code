...
# 创建序列
sequence_length = 300  # 输入序列的长度
output_length = 100  # 输出序列的长度
X, y = [], []
...
# 反归一化预测值和真实值
y_pred_original = scaler.inverse_transform(y_pred.reshape(-1, 1))[:output_length]
y_true_original = scaler.inverse_transform(y_test.reshape(-1, 1))[:output_length]
