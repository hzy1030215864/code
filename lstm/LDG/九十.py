...
# 定义模型
model = Sequential()
model.add(LSTM(units=90, activation='relu', return_sequences=True, input_shape=(X_train.shape[1], X_train.shape[2])))
...
# 训练模型
history = model.fit(X_train, y_train, epochs=100, batch_size=50, validation_split=0.15)
...
print('MSE:', mse)
print('RMSE:', rmse)
print(mape(y_test_reshaped, y_pred_reshaped))  # 你可以继续使用你的 mape 函数