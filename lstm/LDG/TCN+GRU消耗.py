...
# 定义模型
model = Sequential()
model.add(TCN(nb_filters=128, kernel_size=3, nb_stacks=2, dilations=[1, 2, 4, 8, 16, 32], padding='causal', use_skip_connections=True, dropout_rate=0.01, return_sequences=True, activation='tanh'))
...
df_result.to_excel('TCN+GRULDG消耗.xlsx', index=False)
plt.legend()
...
df_metrics.to_excel('TCN+GRULDG消耗数据.xlsx', index=False)
plt.show()